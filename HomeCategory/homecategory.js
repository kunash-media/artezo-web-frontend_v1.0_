...
const res = await fetch(
  ...
  function buildProductCard(p) {
    ...
    // Cart button — mirrors trending pattern exactly
    ...
    const cartBtnStyle = isAdded
      ? `background:#e39f32;color:#1D3C4A;font-weight:600;`
      : "";

    return `
      <div class="product-card bg-white rounded-xl h-[360px] border border-[#fccd81] overflow-hidden shadow-sm hover:-translate-y-1 hover:shadow-md transition-all duration-300 flex flex-col cursor-pointer group ${isOutOfStock ? "grayscale opacity-75" : ""}"
           data-pid="${pid}"
        ...
        <div class="p-3 flex flex-col flex-1">
          ...
          ${color       ? `<span class="text-xs text-gray-500 mb-1">Color: <span class="text-gray-700 font-medium">${color}</span></span>` : ""}
        ...
        <!-- Add to Cart CTA -->
        <button
          class="cart-btn w-full py-2.5 border-t border-[#fccd81] text-sm font-medium bg-gray-50 hover:bg-[#1D3C4A] hover:text-white transition-colors duration-200 flex items-center justify-center gap-2"
          style="${cartBtnStyle}"
          data-pid="${pid}"
          data-variant-id="${escapeHtml(resolvedVariantId)}"
          data-sku="${escapeHtml(resolvedSku)}"
...
//         let url = `/products/product-detail.html?id=${pid}&sku=${sku}&brand=${brandSlug}&category=${categorySlug}`;
